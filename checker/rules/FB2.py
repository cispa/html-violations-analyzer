...
name_informal = """ Leaving out spaces between attributes """
description = """ Check whether an element exists that leaves out spaces between attributes """
...
def check_tag(inp):
    ...
    for c in inp[1:]:
        ...
        if state == "attribute_value_unquoted":
            if c in ["\t", "\n", "\r", "\f", " "]:
                # skipping after state, because we dont need it here
                state = "before_attribute"
                continue
            elif c == ">":
                return True
...
def run_rule(html, debug=True):
    ...
    waf3 = WAF3()
    waf3.feed(html)
    tags = waf3.get_all_tags()
    data = "\n".join(waf3.get_all_data())
